...
void discard_clusters(){
	...
	int clst_per_proc, start_i, start_i_compl, end_i, end_i_compl; //Used for dividing discarding worload.
	...
	// The strategy for obtaining the subset of clusters i is the same one as in the RUBIC algorithm (to balance worload)
	clst_per_proc = (num_bicluster - 1) / 2 / nprocs; // num_bicluster -1 because first cluster is never checked
	start_i = rank * clst_per_proc; end_i_compl = num_bicluster - 2 - start_i;
	end_i = (rank+1) * clst_per_proc - 1; start_i_compl = num_bicluster - 2 - end_i;

	// Now we start the discarding process
	i = start_i;
	while (i <= end_i_compl){
		
		if (rank != nprocs - 1 && i == end_i + 1)
		{
			i = start_i_compl;
		}
